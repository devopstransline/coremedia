...
final class SubmissionStore {
   ...
   */
  String addSubmission(String subject, List<SubmissionContent> submissionContents) {
    Submission submission = new Submission(subject, submissionContents, delayBaseSeconds, delayOffsetPercentage);
    String id = String.valueOf(NEXT_SUBMISSION_ID.getAndIncrement());
  ...
  void cancelSubmission(String submissionId) {
    synchronized (submissions) {
  ...
  TLCSubmissionState getSubmissionState(String submissionId) {
    synchronized (submissions) {
      if (submissions.containsKey(submissionId)) {
  ...
  Collection<Task> getCompletedTasks(String submissionId) {
    synchronized (submissions) {
      if (submissions.containsKey(submissionId)) {
  ...
  Collection<Task> getCancelledTasks(String submissionId) {
    synchronized (submissions) {
      if (submissions.containsKey(submissionId)) {
