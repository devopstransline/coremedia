...
final class Submission {
  ...
  TLCSubmissionState getState() {
    if (atLeastOneTaskCancellationConfirmedRemainingDelivered()) {
      return TLCSubmissionState.CANCELLATION_CONFIRMED;
    }
    if (anyTaskCancelled()) {
      return TLCSubmissionState.CANCELLED;
    }
    if (allTasksDelivered()) {
      return TLCSubmissionState.DONE;
    }

    return TLCSubmissionState.PRODUCTION;
  }
