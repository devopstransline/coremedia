...
public class TlcStudioPluginBase extends StudioPlugin {
  ...
  private static const TRANSLATION_TRANSLINE_PROCESS_NAME:String = "TranslationTransline";
  ...
  private static const TRANSLINE_SUBMISSION_STATUS_VARIABLE_NAME:String = "translineSubmissionStatus";
  ...
  public function TlcStudioPluginBase(config:TlcStudioPluginBase = null) {
    super(config);
  }
  ...
  internal static function transformSubmissionStatus(value:String):String {
    ...
    var status:String = ResourceManager.getInstance().getString('de.transline.labs.translation.tlc.studio.TlcProcessDefinitions', 'TranslationTransline_submission_status_' + value);
    if (!status) {
      status = value;
    }
    return status;
  ...
  internal static function transformSubmissionId(value:Array):String {
    ...
    return ResourceManager.getInstance().getString('de.transline.labs.translation.tlc.studio.TlcProcessDefinitions', 'TranslationTransline_submission_id_unavailable');
  }
    ...
    return function (data:*):String {
      ...
      if (data && data.bean as WorkflowObject) {
        ...
        if (processAvailable(process) && process.isLoaded()) {
          var tlcSubmissionsState:String = process.getProperties().get(TRANSLINE_SUBMISSION_STATUS_VARIABLE_NAME) as String;
          var cancelRequested:Boolean = process.getProperties().get(CANCEL_REQUESTED_VARIABLE_NAME) as Boolean || tlcSubmissionsState === CANCELLATION_CONFIRMED_SUBMISSION_STATE || tlcSubmissionsState === CANCELLED_SUBMISSION_STATE;
          if (cancelRequested) {
  ...
  internal static function getTranslationCancelToolBarButtonTLC(workflowObjectVE:ValueExpression, panel:GridPanel):IconButton {
    var buttonCfg:IconButton = IconButton({});
    buttonCfg.itemId = "cancelButtonID";
    buttonCfg.disabled = !isButtonEnabled(workflowObjectVE);
    buttonCfg.tooltip = ResourceManager.getInstance().getString('de.transline.labs.translation.tlc.studio.TlcProcessDefinitions', 'Action_Tooltip_Cancel_Process');
    ...
    cancelButton.setHandler(function ():void {
      var messageText:String = ResourceManager.getInstance().getString('de.transline.labs.translation.tlc.studio.TlcProcessDefinitions', 'confirm_cancellation');
      var promptTitle:String = ResourceManager.getInstance().getString('de.transline.labs.translation.tlc.studio.TlcProcessDefinitions', 'confirm_cancellation_title');
  ...
  private function getDayOffsetFromSettings(offSetValueExpression:ValueExpression):void {
    var translineSettings:Content = SESSION.getConnection().getContentRepository().getChild("/Settings/Options/Settings/Transline");
    if (translineSettings) {
      translineSettings.load(function ():void {
        var settings:Struct = translineSettings.getProperties().get("settings") as Struct;
        if (settings) {
          var tlcSettingsVE:ValueExpression = ValueExpressionFactory.createFromFunction(function ():Struct {
            var tlcSettings:Struct = settings.get("transline") as Struct;
            if (!tlcSettings) {
              return undefined;
            }
            return tlcSettings;
          ...
          tlcSettingsVE.loadValue(function ():void {
            offSetValueExpression.setValue(tlcSettingsVE.getValue().get("dayOffsetForDueDate") as int);
          });
        }
      });
    }
  ...
  private static function isButtonEnabled(workflowObjectsVE:ValueExpression):Boolean {
    ...
    return workflowObjects && workflowObjects.length && workflowObjects.map(getProcess).every(function(process:Process):Boolean {
      ...
      var isTranslationTransline:Boolean = processAvailable(process) && process.getDefinition().getName() === TRANSLATION_TRANSLINE_PROCESS_NAME;
      ...
      return isTranslationTransline && cancellationAllowed && !cancelAlreadyRequested;
    });
  }
  ...
  internal function getButtons():Function {
    return function (panel:GridPanel, processCategory:String, panelType:String, selectedWorkflowObjectVE:ValueExpression, callback:Function = null):Array {
      ...
      if (panelType === "processListPanel") {
        ...
        buttons.push(getTranslationCancelToolBarButtonTLC(selectedWorkflowObjectVE, panel));
        return buttons;
      }
  ...
  protected function convertLocales(locales:Array):String {
    if (locales) {
      ...
      return ResourceManager.getInstance().getString('de.transline.labs.translation.tlc.studio.TlcProcessDefinitions', 'TranslationTransline_Multi_Target_Locale_Text', [locales.length]);
    }
    return "";
  }
